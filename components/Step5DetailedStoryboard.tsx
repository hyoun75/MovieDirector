...
const Step5DetailedStoryboard: React.FC<Step5DetailedStoryboardProps> = ({
  ...
  const fetchDetailedStoryboard = async () => {
    ...
    try {
      ...
      setError('세부 콘티를 생성하는 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    // Check if detailed scenes are already generated or if they differ significantly in count/structure
    // For simplicity, if we have 0 detailed scenes, generate.
  ...
  const handleDownload = () => {
    let md = `# Detailed Storyboard (Shooting Script) for "${story.title}"\n\n`;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[500px]">
        ...
        <p className="mt-6 text-xl text-slate-300 font-medium">콘티를 5초 단위의 짧은 컷으로 재생성하고 있습니다...</p>
      </div>
    );
  }
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-3xl font-bold flex items-center gap-2">
            <span className="text-indigo-400">#5</span> 세부 콘티 (5초 제한)
          </h2>
          <p className="text-slate-400 mt-1">이전 단계의 콘티를 기반으로 5초 이하의 짧은 컷들로 재생성합니다. 이 세부 콘티가 이미지/영상 프롬프트의 기준이 됩니다.</p>
        </div>
            ...
            <div className="flex flex-col md:flex-row">
              <div className="bg-slate-900/50 p-4 flex flex-col justify-center items-center w-full md:w-32 border-b md:border-b-0 md:border-r border-slate-700">
                ...
                <div className={`flex items-center gap-1 mt-2 text-xs font-mono px-2 py-0.5 rounded ${
                   // Visual feedback for duration compliance
